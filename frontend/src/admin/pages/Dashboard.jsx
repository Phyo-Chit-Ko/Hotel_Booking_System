  ...
  return (
    <AdminLayout>
      {viewState === "view_report" ? (
        ...
        /* Viewport Lock Wrapper: Limits height to exactly the visible frame to eliminate scroll */
        <div className="w-full h-[calc(100vh-110px)] flex flex-col gap-5 overflow-hidden p-1">
          
          {/* 1. Control Header (Highly Scaled and Legible) */}
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 shrink-0">
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">Property Overview</h1>
              <p className="text-sm font-medium text-slate-500 mt-0.5">Real-time terminal environment</p>
            </div>
          ...
          {/* 2. Key Metrics Row (Large Readouts) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 shrink-0">
            {stats.map((stat, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex items-center justify-between">
                ...
                <span className={`text-xs font-bold px-3 py-1 rounded-full ${stat.isPositive ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
                  {stat.change}
                </span>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 flex-1 min-h-0">
            ...
            {/* Left Hand Side: Two-Line Interactive Performance Chart */}
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm lg:col-span-2 flex flex-col justify-between min-h-0">
              <div className="flex justify-between items-center pb-3 shrink-0">
                <div>
                  <h2 className="text-lg font-bold text-slate-900 tracking-tight">Analytical Performance Dynamics</h2>
                  <p className="text-xs text-slate-400 mt-0.5">Correlated performance metrics across the selected matrix timeline</p>
                </div>
              ...
              <div className="relative flex-1 bg-slate-50/50 rounded-xl border border-slate-100 p-4 flex flex-col justify-between min-h-0 overflow-hidden">
                ...
                <div className="relative w-full h-full min-h-0 pt-4">
                  <svg className="w-full h-full overflow-visible" viewBox="0 0 600 160" preserveAspectRatio="none">
                    ...
                    {/* Line 1 (Revenue Metric Layer - Slate Dark Line) */}
                    <path 
                    ...
                    {/* Line 2 (Occupancy Metric Layer - Solid Amber Line) */}
                    <path 
                    ...
                    {/* Plot Points and Coordinates Indicators */}
                  ...
                  {/* Context Anchored Indicator Data Badges */}
                ...
                <div className="flex justify-between text-xs text-slate-500 font-bold pt-2 border-t border-slate-200 shrink-0">
                  <span>01 Jun</span><span>07 Jun</span><span>14 Jun</span><span>21 Jun</span><span>30 Jun</span>
                </div>
              </div>
            </div>

            {/* Right Hand Column: Split Distribution Metrics & System logs */}
            <div className="grid grid-rows-2 gap-5 min-h-0">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between min-h-0">
                <div className="shrink-0">
                  <h3 className="text-base font-bold text-slate-900">Distribution Channels</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Yield breakdown share</p>
                </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between min-h-0">
                <div className="shrink-0">
                  <h3 className="text-base font-bold text-slate-900">Live Activity Log</h3>
                  <p className="text-xs text-slate-400 mt-0.5">System telemetry events</p>
                </div>
                
                {/* Overflow Locked Internal Action Stream List */}
                <div className="flex-1 overflow-y-auto mt-3 pr-1 space-y-2.5 min-h-0 scrollbar-none">
                  {recentActivities.map((act) => (
                    <div key={act.id} className="flex justify-between items-center gap-3 text-xs py-1.5 border-b border-slate-50 last:border-0">
                      <p className="text-slate-700 font-semibold truncate">{act.text}</p>
                      <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded whitespace-nowrap font-mono">{act.time}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-md rounded-2xl border border-slate-200 shadow-2xl relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div>
                <h3 className="text-md font-bold text-slate-900">Compile Report Asset</h3>

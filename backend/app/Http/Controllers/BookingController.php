    ...
    public function update(Request $request, $id)
    {
        ...
        if ($shouldConvert) {
            DB::transaction(function () use ($booking, $room) {
                ...
                ReservationCharge::create(['reservation_id' => $reservation->reservation_id, 'charge_type' => 'room', 'description' => "Room charge — {$nights} night(s)", 'amount' => $roomCharge]);

                if ($extraPersonCharge > 0) {
                    ReservationCharge::create(['reservation_id' => $reservation->reservation_id, 'charge_type' => 'extra_person', 'description' => "Extra person charge — {$nights} night(s)", 'amount' => $extraPersonCharge]);
                }

                if ($taxAmount > 0) {
                    ReservationCharge::create(['reservation_id' => $reservation->reservation_id, 'charge_type' => 'tax', 'description' => 'Tax', 'amount' => $taxAmount]);
                }
